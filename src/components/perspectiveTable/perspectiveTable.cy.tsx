...
const sampleData: Array<
  Record<string, Array<string | boolean | Date | number>>
> = [
  {
    Region: ['East'],
    State: ['New York'],
...
describe('PerspectiveTable Component', () => {
  const data: PerspectiveTableData = {
    data: sampleData,
    config: {
  ...
  it('handles error state', () => {
    const invalidData: PerspectiveTableData = {
      data: [],
    }

    cy.mount(<PerspectiveTable {...invalidData} />)

    cy.get('.rustic-perspective-viewer').should(
      'contain',
